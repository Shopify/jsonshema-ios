...
struct EnumerationValidationError<T>: Error, CustomStringConvertible {
    let value: T
    let expectedValues: [T]
    
    init(value: T, expectedValues: [T]) {
        self.value = value
        self.expectedValues = expectedValues
    }
    
    var description: String {
        let expectedValuesString = self.expectedValues.map { "\($0)" }.joined(separator: ", ")
        return "Value \(self.value) is not in a set of expected values (\(expectedValuesString))"
    }
}
